  ...
  return all
    ...
    .map((d) => {
      ...
      return {
        id: d.id,
        name: names.get(d.id),
        // ── ข้อมูลดิบรายช่อง — ฝั่งจอใช้วาดชื่อยาแบบมีสีทีละส่วน ────────────
        // 🚨 ต้องส่งไปด้วยเสมอ อย่าให้ฝั่งจอไปเดาชิ้นส่วนจากข้อความชื่อ
        //    เคยพลาดมาแล้ว 25 ส.ค. 2569: พอสูตรชื่อเปลี่ยน ตัวเดาก็เดาผิดทันที
        //    ได้ "Morphine sulfate 10 mg/mL injection injection" คำว่า injection ซ้ำสองรอบ
        //    และ (IR) หายไปเฉย ๆ เพราะตัวเดาหาไม่เจอในตำแหน่งที่คาดไว้
        generic: (d.generic || '').trim(),
        strength: (d.strength || '').trim(),
        strengthUnit: (d.unit || '').trim(),
        percent: (d.percent || '').trim(),
