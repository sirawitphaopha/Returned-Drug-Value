...
//    ตอนรันในเครื่องก็ล็อกเหมือนกัน ถ้าตั้ง MRV_PASSWORD ไว้ใน .env.local

export const AUTH_COOKIE = 'mrv_auth';
export const AUTH_DAYS = 30;

// ── 🗑 เคยมีทางลัดปลดล็อกตอนรันในเครื่อง (MRV_DEV_OPEN) — ถอดทิ้งแล้ว 26 ส.ค. 2569 ──
//
// เหตุผลเดิมที่ใส่ไว้: แคลร์กรอกรหัสผ่านลงฟอร์มเองไม่ได้ (ข้อห้ามติดตัว)
// ถ้าเครื่องทดสอบล็อกด้วย ก็เทสหน้าเว็บเองไม่ได้เลย
//
// ทำไมถอดทิ้ง:
//   1. ไม่จำเป็นตั้งแต่แรก — พี่กันกรอกรหัสครั้งเดียว คุกกี้อยู่ 30 วัน เทสต่อได้ทั้งหมด
//   2. มันกัดจริงไปแล้ว — ปุ่ม "ออกจากระบบ" ผูกกับ authEnabled() ซึ่งเป็นเท็จเมื่อปลดล็อก
//      ปุ่มเลยหายไปเงียบ ๆ พี่กันทักว่า "เอาปุ่ม log out เราออกทำไม" เสียเวลาไล่หาอยู่พักใหญ่
//   3. ทำให้เครื่องทดสอบไม่เหมือนเว็บจริง = บ่อเกิดของบั๊ก "ในเครื่องผ่าน เว็บจริงพัง"
//      ประตูรหัสผ่านเป็นของที่ต้องทดสอบมากที่สุด แต่กลับเป็นชิ้นเดียวที่ไม่เคยถูกทดสอบ
//
// 🚨 ห้ามใส่กลับมาอีก — ถ้าเทสไม่ได้เพราะไม่มีคุกกี้ ให้ขอพี่กันกรอกรหัสในเบราว์เซอร์
//    ไม่ใช่เปิดประตูทิ้งไว้
...
// เว็บจริงล็อกเสมอ (ผลตรวจข้อ ส-2) · เครื่องทดสอบล็อกเมื่อตั้งรหัสไว้
// ไม่มีทางลัดปลดล็อกอีกแล้ว — เครื่องทดสอบจึงเดินเส้นทางเดียวกับเว็บจริงทุกขั้น
...
/**
 * บัตรใบนี้ใกล้หมดอายุแล้วหรือยัง — ใช้ตัดสินว่าควรต่ออายุให้เลยไหม
 *
 * 🔑 กฎกลาง `pharmacy-web-logic` ข้อ 24 — ME-DRP ต่ออายุอัตโนมัติทุกครั้งที่ใช้
 *    เว็บนี้เดิมออกบัตรครั้งเดียวแล้วจบ **ใช้ทุกวันก็ยังต้องกรอกรหัสใหม่เมื่อครบ 30 วัน**
 *
 * ทำไมต้องนับจาก "ครั้งสุดท้ายที่ใช้" ไม่ใช่ "วันที่ออกบัตร"
 *   คอมห้องยาเปิดเว็บทุกวัน แต่จู่ ๆ วันหนึ่งก็ถูกเด้งออกกลางงาน
 *   เภสัชกรที่กำลังกรอกยาคืนค้างอยู่ต้องหยุดไปตามหารหัส ซึ่งไม่มีใครจำได้
 *
 * 🚨 ต่ออายุเมื่อ "ใช้ไปเกินครึ่งอายุแล้ว" ไม่ใช่ต่อทุกคำขอ
 *    ต่อทุกคำขอ = เขียนคุกกี้ใหม่หลายสิบครั้งต่อนาที เปลืองเปล่า
 *    และทำให้ตามรอยยาก เพราะเวลาที่ออกบัตรขยับตลอด
 *
 * @param {string} value ค่าคุกกี้
 * @returns {boolean}
 */
...
/** ตัวเลือกของคุกกี้ — ใช้ร่วมกันทั้งตอนออกบัตรและตอนต่ออายุ ห้ามเขียนแยกสองที่ */
