...
export function uiActions(app) {
  ...
  app.goScreen = (name) => {
    // 🚨🔴 ต้องปิดหน้าตั้งค่าด้วยเสมอ (พี่กันเจอบั๊ก 4 ก.ย. 2569)
    //    หน้าตั้งค่าเป็นหน้าเต็มจอที่วาดทับหน้าอื่น ถ้าไม่ปิด กดแท็บแล้วหน้าไม่เปลี่ยน
    //    ดูเหมือนแท็บกดไม่ติดทั้งสี่อัน ทั้งที่ screen เปลี่ยนไปแล้วจริง ๆ
    //    ติดอยู่ในหน้าตั้งค่าออกไม่ได้เลยนอกจากกดปุ่มลูกศรกลับ
    const ปิดตั้งค่า = app.state.settingsOpen ? { settingsOpen: false } : null;

    // ไม่ได้เปลี่ยนหน้าจริง ไม่ต้องไล่จาง (กดแท็บเดิมซ้ำ)
    // แต่ถ้ามาจากหน้าตั้งค่า ยังต้องปิดมันก่อน ไม่งั้นกดแล้วเงียบ
    if (app.state.screen === name) {
      if (ปิดตั้งค่า) app.viewSwap(() => app.setState(ปิดตั้งค่า, () => app.toTop()));
      else app.toTop();
      return;
    }
