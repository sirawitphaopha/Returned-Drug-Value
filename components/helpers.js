...
export const LS = {
  ...
  // คอลัมน์ HN ในหน้าประวัติ เปิดอยู่ไหม (พี่กันสั่ง 10 ก.ย. 2569)
  // 🚨 ค่าตั้งต้นคือซ่อน — HN เป็นข้อมูลผู้ป่วย คนเดินผ่านหลังเคาน์เตอร์ไม่ควรเห็น
  //    และเก็บถาวร ไม่ใช่ที่เก็บของแท็บ เพราะคนที่ใช้ HN จริงจะได้ไม่ต้องกดเปิดทุกวัน
  hnCol: 'mrv.hncol',
...
// ── วัดความกว้างข้อความจริง ๆ ด้วยฟอนต์ที่ใช้บนจอ ────────────────────────────
//
//   พี่กันสั่ง 10 ก.ย. 2569 — "ให้มันเปลี่ยนความยาวตามข้อมูลแหล่งที่มา
//   แต่เรากำหนด fix ตัวสูงสุดไว้ แล้วมันจะทำงานเมื่อ รพ.สต. ที่ยาวที่สุดจะกรอกเท่านั้น"
//
//   คอลัมน์ในตารางกำหนดความกว้างเป็นตัวเลขตายตัว (table-layout: fixed)
//   จะให้กว้างตามข้อมูลที่แสดงอยู่จริง ต้องรู้ก่อนว่าข้อความนั้นกว้างกี่จุด
//
// 🚨 ห้ามเดาจากจำนวนตัวอักษร — ตัวอักษรไทยกว้างไม่เท่ากันเลย
//    "ไฮ" กับ "หนองเชียงทูน" ต่างกันเกือบสามเท่าทั้งที่นับตัวอักษรต่างกันไม่มาก
//
// 🚨🔴 ห้ามใช้ผืนผ้าใบ (canvas) วัด — เคยลองแล้ววัดได้น้อยกว่าของจริง 20%
//    ฟอนต์ในเว็บนี้โหลดผ่าน next/font ซึ่งตั้งชื่อจริงเป็น __Sarabun_xxxxxx
//    เขียน 'Sarabun' ลงไปตรง ๆ ผืนผ้าใบจึงหาไม่เจอ แล้วตกไปใช้ฟอนต์ระบบที่แคบกว่า
//    ผลคือคอลัมน์แคบเกินจริง แล้วข้อความโดนตัด — ซึ่งเป็นสิ่งที่พยายามกันอยู่พอดี
//
//    วัดด้วยกล่องจริงในหน้าเว็บแทน แล้วเขียนฟอนต์เป็น var(--font-xxx)
//    ให้เบราว์เซอร์แปลงชื่อฟอนต์ให้เอง — ตรงกับที่ตารางใช้จริงเสมอ
//
// 🚨 จำผลไว้ใช้ซ้ำ (Map) กล่องจริงจึงถูกวัดครั้งเดียวต่อข้อความหนึ่งชุด
//    ชื่อ รพ.สต. มีแค่ 13 แห่ง + แหล่งอื่นอีก 5 = วัดจริงไม่เกิน 18 ครั้งตลอดอายุหน้า
// 🚨 ตอนเซิร์ฟเวอร์วาดจอยังไม่มีหน้าเว็บ คืน 0 แล้วให้ตัวเรียกใช้ค่าสำรองไปก่อน
//    (พอถึงเบราว์เซอร์จะวัดได้จริงแล้ววาดใหม่เอง)
let _กล่องวัด = null;
const _จำความกว้าง = new Map();
export function textWidth(ข้อความ, ฟอนต์) {
  const t = String(ข้อความ || '');
  if (!t) return 0;
  if (typeof document === 'undefined' || !document.body) return 0;
  const กุญแจ = ฟอนต์ + '|' + t;
  const เคยวัด = _จำความกว้าง.get(กุญแจ);
  if (เคยวัด !== undefined) return เคยวัด;
  try {
    if (!_กล่องวัด) {
      _กล่องวัด = document.createElement('span');
      _กล่องวัด.setAttribute('aria-hidden', 'true');
      _กล่องวัด.style.cssText = 'position:absolute;left:-9999px;top:-9999px;'
        + 'white-space:nowrap;visibility:hidden;pointer-events:none';
      document.body.appendChild(_กล่องวัด);
    }
    _กล่องวัด.style.font = ฟอนต์;
    _กล่องวัด.textContent = t;
    const w = Math.ceil(_กล่องวัด.getBoundingClientRect().width);
    if (w > 0) _จำความกว้าง.set(กุญแจ, w);
    return w;
  } catch (e) { return 0; }
}

/** ฟอนต์ที่ช่องในตารางใช้ — ต้องตรงกับ .tbl tbody td ใน globals.css เป๊ะ ๆ
 *  🚨 ต้องเป็น var(--font-xxx) ห้ามเขียนชื่อฟอนต์ตรง ๆ (ดูเหตุผลข้างบน) */
export const TBL_FONT = '400 13.5px var(--font-en), var(--font-sarabun), Sarabun, sans-serif';

// ── ความสูงแถวตาราง 3 ระดับ (พี่กันสั่ง 10 ก.ย. 2569) ──────────────────────
//   "ความสูง 43 34 30 เรารู้ละ เอาปุ่มเลือกขนาดได้ใส่ไปเลย"
//
// 🚨 ค่าตั้งต้นคือ roomy (43) — ของเดิม 51 จุดเลิกใช้แล้ว สูงเกินจำเป็น
// 🚨 เลขความสูงในนี้เป็นค่าที่วัดได้จริงจากตาราง ไม่ใช่ค่าที่ตั้งใน CSS
//    (CSS ตั้งแค่ระยะบนล่าง ความสูงจริงมาจากปุ่มในแถวเป็นตัวกำหนด)
// 🚨 lines = ตำแหน่งเส้นแนวนอนในไอคอน (พี่กันสั่งให้เป็นไอคอน 10 ก.ย. 2569)
//    ยิ่งเส้นชิดยิ่งแทนแถวที่แน่นขึ้น — ตาอ่านได้ทันทีโดยไม่ต้องแปลจากคำ
export const ROW_H = [
  { key: 'roomy', cls: 'rh-roomy', label: 'โปร่ง', px: 43, hint: 'อ่านสบายที่สุด', lines: [6, 12, 18] },
  { key: 'tight', cls: 'rh-tight', label: 'แน่น', px: 34, hint: 'เห็นเพิ่มราว 7 แถว', lines: [7, 12, 17] },
  { key: 'dense', cls: 'rh-dense', label: 'แน่นมาก', px: 30, hint: 'ตัวอักษรเล็กลง เห็นเพิ่มราว 9 แถว', lines: [8, 11.5, 15, 18.5] }
];
export const ROW_H_KEYS = ROW_H.map((r) => r.key);
export const ROW_H_CLASSES = ROW_H.map((r) => r.cls);
...
// 🚨 ห้ามตัดค่าให้เหลือเพดานเงียบ ๆ (พี่กันเลือกแบบ ก · 1 ก.ย. 2569)
//
//   ของเดิมพิมพ์ 3000000000 แล้วมันกลายเป็น 100000 เฉย ๆ ไม่บอกอะไรเลย
//   พี่กันเพิ่งรู้ตอนลองเล่น — ถ้าเป็นการกรอกจริงอาจไม่ทันสังเกตว่าเลขเปลี่ยน
//   ซึ่งอันตรายกว่าไม่กั้น เพราะเลขนี้ไปโผล่ในรายงานที่เสนอผู้บริหาร
//
//   ตอนนี้คืนค่าที่พิมพ์จริง แล้วให้หน้าจอเตือนกับปิดปุ่มแทน (ดู qtyOver)
export function qtyNum(raw) {
  const n = Math.round((parseFloat(raw || '0') || 0) * 100) / 100;
  return n > 0 ? n : 0;
}

// เกินเพดานไหม — ใช้ตัดสินว่าจะเตือนและปิดปุ่มหรือยัง
// 🚨 เพดานมีไว้กันพิมพ์พลาด (กดเลข 0 ค้าง) ไม่ใช่กันมูลค่า
//    ยาที่คืนจริงต่อครั้งอย่างมากหลักพันเม็ด · ฝั่งเซิร์ฟเวอร์กั้นเลขเดียวกันอีกชั้น
export function qtyOver(n) {
  return Number(n) > MAX_QTY;
}
...
export function evalQty(raw) {
  ...
  return sum > 0 ? sum : 0;                                 // ไม่ตัดเพดานแล้ว หน้าจอเตือนแทน (ดู qtyOver)
}
...
// ── ตารางชั้นของทุกอย่างที่ลอยอยู่เหนือหน้าเว็บ ────────────────────────────
//
// พี่กันเจอเอง 1 ก.ย. 2569: กดลบในหน้าต่างรายการล็อต แล้วป๊อปยืนยันไปโผล่ "ข้างหลัง"
...
//   ป๊อปยืนยันอยู่ที่ 40 ส่วนหน้าต่างรายการล็อตอยู่ที่ 60 จึงถูกทับ
//   เพิ่มหน้าต่างใหม่ทีไรก็ต้องเดาเลขเอง แล้วทับกันไปเรื่อย ๆ
//
// 🚨 ทุกหน้าต่างต้องใช้เลขจากตารางนี้เท่านั้น ห้ามเขียนเลขเองอีก
// 🚨 ฉากหลัง (backdrop) ใช้เลขตรง ๆ · ตัวกล่องใช้ +1 · ของที่ซ้อนในกล่องใช้ +2
// 🚨 ยืนยันต้องสูงกว่าหน้าต่างทุกตัวเสมอ — มันคือด่านสุดท้ายก่อนของหาย
//    ข้อความเด้งสูงกว่ายืนยันอีก เพราะต้องเห็นผลลัพธ์ไม่ว่าอะไรเปิดอยู่
export const Z = {
...
export const APP_VERSION = '0.20.1.0';
export const APP_FIRST_RELEASE = '4 สิงหาคม 2569';
export const APP_LAST_UPDATE = '10 กันยายน 2569';

// ชื่อผู้บันทึกแบบสั้น — คำนำหน้า + ชื่อ ไม่เอานามสกุล
// พี่กันสั่ง 4 ก.ย. 2569: "เอาแค่ตำแหน่ง และชื่อ ไม่เอานามสกุล"
// 🔴 แคลร์เคยตีความผิดว่า "ชื่ออย่างเดียว" คือให้ตัดคำนำหน้าออกด้วย
//    ที่ถูกคือ ภก. สิรวิชญ์ ไม่ใช่ สิรวิชญ์ — คำนำหน้าบอกวิชาชีพ ตัดทิ้งไม่ได้
// เดิมชื่อเต็มยาวจนตกสองบรรทัดในตาราง ทำให้แถวสูงขึ้นทั้งแถว
//
// 🚨 ใช้กับ "สิ่งที่แสดงบนจอ" เท่านั้น ห้ามเอาไปทับค่าที่เก็บลงฐาน
//    ชื่อเต็มเป็นข้อมูลสืบกลับว่าใครเซ็นรับล็อตนั้น ต้องอยู่ครบใน mr_return.recorded_by
//    ไฟล์ส่งออก CSV และใบสรุปที่พิมพ์ ก็ยังใช้ชื่อเต็มเหมือนเดิม
//    บนตารางให้เอาเมาส์ชี้ (title) แล้วเห็นชื่อเต็ม
export function shortStaffName(full) {
  const t = String(full || '').trim();
  if (!t) return t;
  const w = t.split(/\s+/);
  if (w.length === 1) return t;            // มีคำเดียวอยู่แล้ว คืนตามเดิม
  // คำแรกลงท้ายด้วยจุด = คำนำหน้า (ภก. · ภญ.) เอาคำนำหน้ากับชื่อ
  // ไม่มีคำนำหน้า เอาคำแรกคำเดียว
