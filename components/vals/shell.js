...
export function shellVals(app, d) {
  ...
  const V = {
    ...
    wide: d.wide,
    narrow: !d.wide,
    // 🚨 แถบบันทึกล่างจอต้องหายไปตอนเปิดหน้าตั้งค่า
    //    ไม่งั้นมันลอยทับหน้าตั้งค่าอยู่ ทั้งที่คนละหน้ากันแล้ว
    recordNarrow: st.screen === 'record' && !d.wide && !st.settingsOpen,
    ...
    // ── มีหน้าต่างซ้อนเปิดอยู่ไหม (พี่กันสั่ง 1 ก.ย. 2569) ────────────────────
    //   "เราบอกเเล้วว่าพอมี popup เเล้วฉากหลังห้ามเลื่อน"
    //
    // 🚨 ต้องนับให้ครบทุกตัว — ของเดิมนับแค่ 3 ตัวและไม่มีใครเอาไปใช้เลย (โค้ดตาย)
    //    ตกไปตัวเดียว = เปิดหน้าต่างนั้นแล้วฉากหลังยังเลื่อนได้
    // 🚨 เพิ่มหน้าต่างใหม่เมื่อไหร่ ต้องมาเติมที่นี่ด้วยเสมอ
    anyModalOpen: !!(
