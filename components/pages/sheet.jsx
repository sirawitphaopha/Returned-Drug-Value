...
export function renderSheet(V) {
  ...
  return (
    <>
      ...
      <div style={s('position:fixed;left:0;right:0;bottom:0;z-index:21;display:flex;justify-content:center;transform:translateY(calc(var(--kb) * -1));transition:transform .12s ease-out')}>
          ...
          {V.sheetIsOff && (
            <div style={s('background:#f6f7f4;border-radius:11px;padding:11px 12px;margin-bottom:13px')}>
              ...
              <div style={s('display:flex;gap:7px')}>
                ...
                <input value={V.offPrice} onChange={V.onOffPrice} inputMode="decimal" placeholder="ราคา/หน่วย" style={s("flex:1;min-width:0;height:40px;padding:0 11px;border:1px solid rgba(30,36,32,.16);border-radius:9px;background:#fff;font:400 13.5px 'IBM Plex Sans Thai',sans-serif")} />
              </div>
            </div>
          )}
