  ...
  return (
    <div role="dialog" aria-modal="true"
      ...
      <div style={s('background:#fff;border-radius:14px;width:100%;max-width:470px;padding:20px 22px;box-shadow:0 8px 30px rgba(20,26,22,.2)')}>
        ...
        <div style={s('font:400 12.5px/1.6 Sarabun,sans-serif;color:#6b746e;margin-bottom:14px')}>
        ...
        {/* ── ชั้นที่ 1 ── */}
        <div style={s('font:600 11.5px Sarabun,sans-serif;color:#414a44;margin-bottom:5px')}>กรอกจากอะไร</div>
        <div style={s('font:700 11px Sarabun,sans-serif;color:#b02a5b;margin-bottom:9px')}>
        ...
        <div style={s('display:flex;gap:8px;margin-bottom:14px')}>
          {KIND.map((k) => (
            <div key={k.n} {...kb(() => V.setDeviceKind(k.n))}
              ...
              <div style={sx('font:700 13.5px Sarabun,sans-serif;margin-bottom:2px', { color: kind === k.n ? '#2f7d5d' : '#1e2420' })}>{k.title}</div>
              <div style={s('font:400 11px Sarabun,sans-serif;color:#6b746e')}>{k.why}</div>
            </div>
          ))}
        </div>
        ...
        {kind > 0 && (
          <>
            <div style={s('font:600 11.5px Sarabun,sans-serif;color:#414a44;margin-bottom:5px')}>
            ...
            <select value={V.devicePick} onChange={(e) => V.setDevicePick(e.target.value)}
              ...
              style={s('width:100%;height:40px;border-radius:9px;border:1px solid rgba(30,36,32,.18);padding:0 11px;font:500 13px Sarabun,sans-serif;color:#1e2420;background-color:#fff;margin-bottom:11px')}>
              <option value="">{kind === 1 ? '− เลือกเครื่อง −' : '− เลือกชื่อ −'}</option>
              {list.map((x) => <option key={x} value={x} style={s('font:500 13px Sarabun,sans-serif')}>{x}</option>)}
            </select>

            <div style={s('background:#fdf8ec;border:1px solid rgba(150,101,15,.24);border-radius:9px;padding:9px 11px;font:500 11.5px/1.55 Sarabun,sans-serif;color:#7a6033;margin-bottom:14px')}>
              {kind === 1
            ...
            {kind === 2 && !list.length && (
              <div style={s('background:#fdf3f5;border:1px solid rgba(176,42,91,.24);border-radius:9px;padding:9px 11px;font:500 11.5px/1.55 Sarabun,sans-serif;color:#b02a5b;margin-bottom:14px')}>
        ...
        <div style={s('display:flex;gap:8px;justify-content:flex-end')}>
          <div {...(ready ? kb(() => V.pickDevice(V.devicePick)) : {})}
            ...
            style={sx('padding:10px 20px;border-radius:9px;font:700 13px Sarabun,sans-serif;min-height:42px;display:flex;align-items:center', ready
