    ...
    <div style={sx('margin:0 auto;display:flex;flex-direction:column;min-height:100%', { maxWidth: V.pricesWidth })}>
      <div style={s('padding:16px 20px 14px;background:#fff;border-bottom:1px solid rgba(30,36,32,.07)')}>
        <div style={s('display:flex;align-items:center;gap:10px;margin-bottom:12px')}>
          <div onClick={V.closePrices} className="hv-bg-f6" style={s('width:34px;height:34px;border-radius:8px;border:1px solid rgba(30,36,32,.14);display:flex;align-items:center;justify-content:center;font:400 16px Sarabun,sans-serif;color:#6b746e;cursor:pointer;flex:none')}>←</div>
        ...
        <div style={s('display:flex;gap:6px;margin-top:10px;flex-wrap:wrap')}>
          {V.priceFilters.map((f) => (
            <div key={f.key} onClick={f.pick} style={sx('padding:7px 14px;border-radius:999px;font:500 12.5px Sarabun,sans-serif;cursor:pointer', { background: f.bg, color: f.fg })}>{f.label}</div>
          ))}
        </div>
      </div>
      ...
      <div style={s('padding:14px 20px 18px;flex:1')}>
        ...
        {V.priceHasMore && (
          <div onClick={V.morePrices} className="hv-bg-f6" style={s('margin-top:10px;height:44px;border-radius:11px;border:1px solid rgba(30,36,32,.14);display:flex;align-items:center;justify-content:center;font:600 13.5px Sarabun,sans-serif;color:#414a44;cursor:pointer')}>{V.priceMoreLabel}</div>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div style={s('flex:none;background:#fff;border-top:1px solid rgba(30,36,32,.08);box-shadow:0 -6px 20px rgba(30,36,32,.06);order:1;position:relative;z-index:5')}>
      <div style={sx('margin:0 auto;padding:12px 20px max(16px,env(safe-area-inset-bottom));display:flex;align-items:center;gap:10px', { maxWidth: V.pricesWidth })}>
        <div onClick={V.resetPriceEdits} className="hv-bg-f6" style={s('height:50px;padding:0 18px;border-radius:12px;border:1px solid rgba(30,36,32,.16);display:flex;align-items:center;justify-content:center;font:600 14px Sarabun,sans-serif;color:#414a44;cursor:pointer;flex:none')}>ยกเลิก</div>
        <div onClick={V.savePrices} style={sx('flex:1;height:50px;border-radius:12px;color:#fff;display:flex;align-items:center;justify-content:center;font:600 16px Sarabun,sans-serif;cursor:pointer', { background: V.priceSaveBg })}>{V.priceSaveLabel}</div>
      </div>
    </div>
  );
}
